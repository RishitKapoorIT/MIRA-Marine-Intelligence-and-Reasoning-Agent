...
export function LocationProvider({ children }) {
   ...
   * Matches against known ports first, then falls back to Nominatim OSM geocoder.
   */
  const resolveLocationFromText = async (text) => {
    if (!text || typeof text !== 'string') return null;
    const lower = text.toLowerCase();

    // 1. Direct match with known maritime locations
    ...
    // 2. Extract potential city/place keyword (e.g. "near Kolkata", "in Goa", "around Mumbai")
    const match = lower.match(/(?:near|around|at|in|to|off|basin|harbor|port)\s+([a-zA-Z\s]{3,25})/i);
